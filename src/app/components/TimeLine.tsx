  ...
  return (
    <div className='flex flex-col sticky top-10 h-[calc(100vh-80px)]'>
      {sections.map((section) => (
        <div
          key={section.id}
          className='transition-all duration-500 ease-out flex justify-center items-center gap-5 relative'
          id={`link-to-${section.id}`}
        >
          <div className='w-[2px] bg-black/80 flex h-full relative justify-center'/>
          ...
          <a
            href={`#${section.id}`}
            className='transition-all duration-500 ease-out flex justify-center items-center font'
          >
            {section.name}
          </a>
        </div>
